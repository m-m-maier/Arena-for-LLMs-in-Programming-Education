...
type RestOf<T extends IPrompt | NewPrompt> = Omit<T, 'timestamp'> & {
...
export class PromptService {
	...
	protected convertDateFromClient<T extends IPrompt | NewPrompt | PartialUpdatePrompt>(prompt: T): RestOf<T> {
		return {
			...prompt,
			timestamp: prompt.timestamp?.toJSON() ?? null,
		};
	}
	...
	protected convertDateFromServer(restPrompt: RestPrompt): IPrompt {
		return {
			...restPrompt,
			timestamp: restPrompt.timestamp ? dayjs(restPrompt.timestamp) : undefined,
		};
	}
